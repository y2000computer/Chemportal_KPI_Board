      ...
      <v-row
        ><v-col>
          ...
          >
            ...
            <template v-slot:item.SITEDESC="{ item }">
              {{ item.SITEDESC | shorten(20) }}
            </template>
  ...
  methods: {
    ...
    getRushProject(id) {
      ...
      this.$axios.get('vips/rushproject/' + id).then((response) => {
        ...
        this.form.courier = response.data.data.COURIER;
        ...
        this.form.sitedesc = response.data.data.SITEDESC;
