      ...
      <v-row
        ><v-col>
          ...
          >
            ...
            <template v-slot:item.LABEL_GROUP="{ item }">
              {{ item.LABEL_GROUP | shorten(30) }}
            </template>            
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        { text: 'Split Label Group', value: 'LABEL_GROUP' },        
  ...
  methods: {
    ...
    saveSetup() {
      this.$validator.validateAll().then((result) => {
        ...
        if (this.editing) {
          this.$axios
            .patch('phaseone/watersplittingsetup/' + this.setup.ID, this.form)
            .then((response) => {
              this.savedResponse();
            });
        } else {
          //this.$axios.post('vips/rushproject', this.form).then((response) => {
          this.$axios.post('phaseone/watersplittingsetup', this.form).then((response) => {
            this.savedResponse();
          });
        }
      });
    ...
    getSetup(id) {
      ...
      this.$axios.get('phaseone/watersplittingsetup/' + id).then((response) => {
        ...
        this.form.label_group = response.data.data.LABEL_GROUP;
