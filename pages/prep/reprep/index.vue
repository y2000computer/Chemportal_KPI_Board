    ...
    <v-col>
      <v-card outlined>
        ...
        <v-card-text>
          ...
          <v-row justify="space-between">
            <v-col v-if="this.other_reason_is" md="5" cols="3">
             <v-text-field
                outlined
                dense
                label="Reason notes"
...
export default {
  ...
  methods: {
    ...
    ReasonSelected() {
      if(this.reprep_reason_master_id == 7) {
        this.other_reason_is=true;
      } else {
          this.other_reason_is=false;
      }
    ...
    getSample_and_update() {
      ...
      if(this.reprep_reason_master_id == 7) {
        if(this.reason_note==''){ 
          this.searchError = 'Please input Reason note';
          return;
        }
      }
