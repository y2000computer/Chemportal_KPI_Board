...
export default {
  ...
  methods: {
    ...
    getSample_and_update() {
      ...
      this.sampleLoading = true;
      this.$axios.get('laboratory/sample/' + this.search).then((response) => {
      //this.$axios.get('laboratory/traycontainer/' + this.search).then((response) => {  
        this.sample = response.data.data;
        this.sampleLoading = false;
        this.search = '';
    ...
    async getStrayV2() {
      ...
      await this.$axios.get('laboratory/straycontainer/' + this.searchbystray).then((response) => {
        ...
        this.stray_samples_loop_counter = 0;
        if(this.stray_samples.length > 0)     
        {
            ...
            this.asyncForEach(this.stray_samples, async (row) => {
                  this.stray_samples_loop_counter = this.stray_samples_loop_counter + 1;
                  this.seq = this.seq + 1;
                  ...
                  await this.$axios
                          ...
                          .then((response) => {
                              ...
                              if(this.stray_samples_loop_counter  >= this.stray_samples.length) {
                                this.progress_message="Prep List All Samples had timestamp already "       
                                this.resetForm(); 
                                this.getSamplesCpts();
                                      }
