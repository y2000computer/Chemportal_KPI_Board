      ...
      <v-card flat>
        <v-toolbar dense flat>
          <v-toolbar-title class="font-weight-medium">
            Job Exception Approval</v-toolbar-title
          >
          <v-spacer />
            ...
            <v-col>
              ...
              >
                ...
                <template v-slot:item.referred="{ item }">
                  <v-chip small :color="item.referred > 0 ? 'orange' : ''">{{
                    item.referred > 0 ? 'Y' : 'N'
                  }}</v-chip>
                </template>
                <template v-slot:item.SKIP="{ item }">
                  <v-chip
                    small
                    :color="item.SKIP == 1 ? 'red lighten-1' : ''"
                    >{{ item.SKIP == 1 ? 'Y' : 'N' }}</v-chip
                  >
                </template>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'With Lab', value: 'referred' },
        { text: 'Skip Auto', value: 'SKIP' },
