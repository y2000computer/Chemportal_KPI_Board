      ...
      <v-card flat>
        <v-toolbar dense flat>
          <v-toolbar-title class="font-weight-medium">
            Pre-bounced Job Exception Lab Approval</v-toolbar-title
          >
          <v-spacer />
            ...
            <v-col>
              ...
              >
                ...
                <template v-slot:item.office.client.CLIENTNAME="{ item }">{{
                  item.office.client.CLIENTNAME | shorten(5)
                }}</template>
                ...
                <template v-slot:item.NO_SAMPLES="{ item }">{{
                  item.NO_SAMPLES | shorten(5)
                }}</template>
                ...
                <template v-slot:item.actions="{ item }">
                  <v-btn
                    :disabled="item.EXCEPTIONS == -1"
                    icon
                    :to="'/technical/pre_bounced_acknowlege/' + item.JOB"
                  >
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Ttl Ex.', value: 'EXCEPTIONS' },
        { text: 'Ttl Apl.', value: 'EXCEPTIONS_AD' },
        { text: 'Inorgan.O', value: 'EX_INORGANICS_EXCEPTIONS_OUT' },
        { text: 'Organ.O', value: 'EX_ORGANICS_EXCEPTIONS_OUT' },
        { text: 'Prep.O', value: 'EX_PREP_EXCEPTIONS_OUT' },
        { text: 'Tech.O', value: 'EX_TECHNICAL_EXCEPTIONS_OUT' },
        { text: 'Asbe.O', value: 'EX_ASBESTOS_EXCEPTIONS_OUT' },
