      ...
      <v-card flat>
        <v-toolbar flat dense>
          <v-toolbar-title class="font-weight-medium"
            >Print Sample Labels</v-toolbar-title
          >
        </v-toolbar>
        ...
        <v-card-text>
          <v-row>
            ...
            <v-col md="4" cols="12" class="text-center">
              ...
              <v-btn-toggle dense v-if="$vuetify.breakpoint.name != 'xs'">
                <v-btn
                  width="180"
                  v-if="job.JOB"
                  @click.prevent="setAllPrint(false)"
                >
                  Select None
                ...
                <v-btn
                  width="180"
                  v-if="job.JOB"
                  @click.prevent="setAllPrint(true)"
                >
                  Select All
              ...
              <div v-if="$vuetify.breakpoint.name == 'xs'">
                ...
                <v-row dense>
                  <v-col cols="12">
                    ...
                    >
                      Priority
                      <v-icon v-if="priorityLabels" class="ml-2"
                        >check_box</v-icon
                      >
            ...
            <v-col md="9" cols="12">
              <v-card flat>
                ...
                <v-card-text>
                    ...
                    <v-col md="3" cols="12">
                      <v-list>
                        <v-list-item>
                          <v-list-item-content>
                            <v-list-item-title class="font-weight-bold"
                              >Sample Details</v-list-item-title
                            >
                          </v-list-item-content>
                        </v-list-item>
                      </v-list>
                          ...
                          <v-list-item-action>
                            <v-btn-toggle rounded>
                              <v-btn
                                small
                                icon
                                @click="setContainersPrint(false)"
                              >
                  ...
                  <div
                    v-for="(sample, index) in job.samples"
                    :key="sample.SAMPNUM"
                  >
...
export default {
  ...
  methods: {
    ...
    jobSearch(job) {
      ...
      this.$axios
        ...
        .then(() => {
          ...
          this.setPrepPrint_after_job_search(true);
        })
    ...
    setPrepPrint_after_job_search(value) {
      this.job.samples.forEach((sample, sampIndex) => {
        sample.prep_labels.forEach((prepLabel, prepIndex) => {
          //console.log('sample id = ',this.job.samples[sampIndex].SAMPNUM)
          //console.log('sample.TYPE = ',this.job.samples[sampIndex].TYPE)
          //console.log('name= ',this.job.samples[sampIndex].prep_labels[prepIndex].name)
          //console.log('subcon= ',this.job.samples[sampIndex].prep_labels[prepIndex].subcon)
          this.prep_label_default_is = true
          if(this.job.samples[sampIndex].TYPE == 'WATER' && this.job.samples[sampIndex].prep_labels[prepIndex].name == 'Asbestos' ) {
            this.prep_label_default_is = true
          } else if(this.job.samples[sampIndex].TYPE == 'WATER' && this.job.samples[sampIndex].prep_labels[prepIndex].subcon == false ) {
            this.prep_label_default_is = false
          }

          this.$set(
            this.job.samples[sampIndex].prep_labels[prepIndex],
            'print',
            this.prep_label_default_is
          );
        });
      });
    },
    ...
    clientPriorityCheck() {
      if (
        this.job.office.client.client == 'AA ENVIRONMENTAL L-P' ||
        this.job.office.client.client == 'ERITH CONTRACTORS -D'
      ) {
        this.priorityLabels = true;
      } else {
        this.priorityLabels = false;
      }
    },
    addTrays() {
    ...
    printLabels() {
      ...
      this.job.samples.forEach((sample) => {
        sample.containers.forEach((container, container_index) => {
          ...
          if (container.print == true && this.job.EXTSTORAGE == 1) {
            this.printData.csvRows.push([
              this.job.JOB,
              '',
              this.job.office.client.CLIENTNAME,
              ...
              sample.SAMPBOTTOM,
              this.job.logged_by.USER_NAME,
              this.$moment(this.job.LOGDATE, 'YYYY-MM-DD').add("days", 28).format('DD/MM/YYYY'),
              this.$moment(this.job.OFFICEDATE, 'YYYY-MM-DD').format(
                'DD/MM/YYYY'
              ),
              this.$moment(this.job.DISPDATE, 'YYYY-MM-DD').format(
              ...
              '',
              this.job.EXTSTORAGE,
              'NM_CONTAINER_DISPOSE_LABEL.BTW',
              this.selectedPrinter,
              container_index + 1 + '/' + sample.containers_count,
              '',
            ]);
          ...
          if (prepLabel.print == true) {
            prepLabels.push([
              ...
              prepLabel.subcon
                ...
                : prepLabel.name == 'Water Running Label'
                ? this.$moment(this.job.INSDATE, 'YYYY-MM-DD').format(
                    'DD/MM/YYYY'
                  )
                : this.$moment(this.job.OFFICEDATE, 'YYYY-MM-DD').format(
                    'DD/MM/YYYY'
              ...
              this.job.EXTSTORAGE,
              prepLabel.name == 'Water Running Label'
                ? 'NM_WATER_RUNNING.BTW'
                : 'NM_PREP_LABEL_NEW.BTW',
              this.selectedPrinter,
              '',
              prepLabel.name == 'Water Running Label'
                ? 'prep' +
